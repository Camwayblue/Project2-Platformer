...
function Coin(pos) {
  ...
  this.size = new Vector(0.3, 0.6);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
Coin.prototype.type = "coin";

function Invincable(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0, 0));
  this.size = new Vector(0.7, 0.7);
}
   Invincable.prototype.type = "invincable";
...
function Bat(pos) {
  ...
  // Make it go back and forth in a sine wave.
  this.fly = Math.random() * Math.PI * 2;
}
...
var flySpeed = 2, flyDist = 2;

Bat.prototype.act = function(step) {
  this.fly += step * flySpeed;
  var flyPos = Math.sin(this.fly) * flyDist;
  this.pos = this.basePos.plus(new Vector(.5, flyPos));
};


Invincable.prototype.act = function(step) {
...
Level.prototype.playerTouched = function(type, actor) {
  ...
  if (type == "lava" && this.status == null) {
    ...
    this.status = "lost";
    this.finishDelay = 1;
	 } else if (type == "invincable" && this.status == null) {
         this.status = "invinc";
